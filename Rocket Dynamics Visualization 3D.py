...
##     Files_path = r'F:\fetchen\My Documents\Rocketry\RAF Data Logger\My Flights\SpeedRunner 54 10_18_15'   # the r indicates to include the \ as is
    ...
    delta_t = .0007     # interval between Rocket Logger samples
    LCG = .66           
    maxrange = 15000    # scales the trajectory plot

    baro_delta_t = .05  # interval between Stratologger samples
    baro_read_interval = int(baro_delta_t/delta_t)
    baro_read_counter = 1
    delta_Z_max = 100   # set the maximum change threshold in altitude for a single altitude interval (to ignore deployment charge spike)

    gps_delta_t = 1
    gps_read_interval = int(gps_delta_t/delta_t)
    gps_read_counter = 1
