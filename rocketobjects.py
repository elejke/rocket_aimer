# rocketobjects - a collection of graphical rocket objects using visual
# Tom Fetter @2015
# version 1.1
...
class fin(object):
    ...
    @property
    # Axis is aligned with the root edge of the fin (along th elength of the rocket)
    def axis(self):
        return self._axis
    @axis.setter
    def axis(self,axis):
...
class rocket(object):
    def __init__(self, frame=None, Dr=.1,Lbt=.66, Lnc=.33, cr=.12, ct= .08, s=.12, Tf=.01, LCG=.7,
                 pos=(0,0,0), axis=(0,0,1), up=(1,0,0), CGtrail=False, Tiptrail=False):
        ...
        #two frames are used to align the rocket's z azis with "axis" and the x axis with "up"             
        ...
        self._fin1 = fin(frame=self._frame2, cr=cr, ct=ct, s=s, Tf=Tf, pos=vector(Dr/2,0,-self._L2), axis=(0,0,1), up=(1,0,0), color=color.yellow)
        self._fin2 = fin(frame=self._frame2, cr=cr, ct=ct, s=s, Tf=Tf, pos=vector(0,Dr/2,-self._L2), axis=(0,0,1), up=(0,1,0), color=color.red)
        self._fin3 = fin(frame=self._frame2, cr=cr, ct=ct, s=s, Tf=Tf, pos=vector(-Dr/2,0,-self._L2), axis=(0,0,1), up=(-1,0,0), color=color.red)
        self._fin4 = fin(frame=self._frame2, cr=cr, ct=ct, s=s, Tf=Tf, pos=vector(0,-Dr/2,-self._L2), axis=(0,0,1), up=(0,-1,0), color=color.red)
    ...
    def Dr(self,Dr):
        ...
        self._fin4.pos = vector(0,-Dr/2,-self._L2)    
    ...
    def Lbt(self,Lbt):
        ...
        self._fin4.pos = vector(0,-self._Dr/2,-self._L2)   
...
if __name__ == '__main__':
    ...
    scene1=display(title="Rocket Orientation", width=600, height=600)
